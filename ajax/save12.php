...
if($bongobon) if (empty($StapB1 || $StapB2 || $StapB3 || $StapBW)){
    $err++;
...
if (($bongobon) && (strlen($bcode) < 3)) {
	$err++;
	$err_msg .= "- Bongobon Number is too short.\r\n";
}
...
//echo "SELECT `available_date` FROM user_availability WHERE user='$uid' ORDER BY available_date ASC";
//exit;
$result ='';
  $flag='';
...
// New UPDATE Mail(Admin)     
        ...
        $message_ad.="Een student updated zijn profiel met de volgende informatie:\n";
		$message_ad.="First Name : $sql_first_name\n";
        $message_ad.="Last Name : $sql_last_name\n";
        if($B!=''){
        	$message_ad.="Steps:$stap,$B\n";
    	}else{
    		$message_ad.="Steps:$stap\n";
    	}
        $message_ad.="Cellphone number: $sql_phone\n";
        $message_ad.="Available On: $result\n";
        $message_ad.="Language: $lang\n";
		$message_ad.="E-mail: $sql_email\n";
		$message_ad.="Distance to club in minutes: $sql_traveltime\n";
        $message_ad.="Age: $age\n";
        $message_ad.="Message : $sql_message\n";
        $message_ad.="Kind regards,\n";
        $message_ad.="Kitesurf.be - 0472 / 94.99.71 \n";
